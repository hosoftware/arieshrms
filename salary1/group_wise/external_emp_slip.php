...
require_once('fpdf_protection.php');
    ...
    e.id as employee_id,e.Name, e.Designation, e.Email,e.employee_code,e.sub_division_id,e.nationality, s.*, YEAR(u.dob) AS dob_year
    ...
    WHERE e.Email!='' AND e.Email!='NA'AND s.month= '$month'AND s.year= '$year' AND e.working=1 AND u.user_id = $user_id";
    ...
    $user_password  = $trans_row['dob_year'] ?? '0000'; 
    ...
    $pdf->Output('Salary_slip_with_deduction_'.$trans_row['employee_code'].'.pdf', 'D');
