...
$currentMonth = date('F',strtotime($date));
 ...
 $sql = "SELECT  s.id AS id, e.esi_no, e.pf_no, e.pan_no,e .wff_no,
        IF(e.DOJ='0000-00-00','',DATE_FORMAT(e.DOJ,'%d/%m/%Y')) AS DOJ,
        e.employee_code,e.id AS employee_id,e.Name,d.full_name AS company_name,
        d.address, d.po_box, d.city, d.telephone, d.fax, d.email AS com_email, e.company_id, 
        e.Designation, e.Email, YEAR(u.dob) AS dob_year, s.*
	    FROM `0_cochin_salary` s
        LEFT JOIN `0_emp` e ON e.id=s.emp_id 
        LEFT JOIN `0_dimensions` d ON d.id=e.company_id 
...
$user_password  = $row['dob_year']; // e.g. "EMP001"
...
$pdf->SetFont('Times','B',18);
 ...
 $pdf->image('../reporting/aries_logo.jpg', 14, 17, 50);   
//}

$pdf->image('../reporting/Effismnew.jpg', 165, 14, null, 15);
$pdf->Cell(190,15,'',0,0,'C');
$pdf->Ln();
