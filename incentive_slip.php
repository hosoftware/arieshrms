...
$inc_month = $_GET['inc_month'] ?? '01';
$inc_year = $_GET['inc_year'] ?? '';
$dimension_id = $_GET['dimension_id'] ?? '';
$dimension2_id = $_GET['dimension2_id'] ?? '';
$dimension3_id = $_GET['dimension3_id'] ?? '';
...
$currency_result=db_query("select * from 0_currencies");
...
while ($currency_row = db_fetch_assoc($currency_result)) {
   
    
    
    $curr_code=$currency_row['curr_abrev'];
    $rate= get_exchange_rate_from_home_currency($curr_code, $rate_date);
    $currency_array[$curr_code]=$rate;
}

if(isset($_GET['loop']) && $_GET['loop']=="success"){
    $sql = "SELECT e.sub_division_id, e.employee_code,e.id as employee_id,e.Name ,e.Bank_Name,e.IBAN,e.Designation,e.division_id,e.company_id,i.*,d.company_currency, i.inc_month, i.inc_year
FROM 0_emp_incentive i left join  0_emp e  on e.id=i.employee_id left join  0_dimensions d  on d.id=e.company_id" ;
if(!empty($ids)){
 $sql.= " where i.id in ($ids)   ";
}
}else{
 $sql = "SELECT  e.sub_division_id,e.employee_code,e.id as employee_id,e.Name ,e.Bank_Name,e.IBAN,e.Designation,e.division_id,e.company_id,i.*,d.company_currency , i.inc_month, i.inc_year
FROM 0_emp_incentive i left join  0_emp e  on e.id=i.employee_id left join  0_dimensions d  on d.id=e.company_id  where i.inc_month= '$inc_month'  AND i.inc_year= '$inc_year'   ";
}
if ($dimension_id > 0)
    $sql.=" and e.company_id='$dimension_id'";
if ($dimension2_id > 0)
    $sql.="  and e.division_id='$dimension2_id'";
if ($dimension3_id > 0)
    $sql.="  and e.sub_division_id='$dimension3_id'";
if ($employee_id > 0)
...
$sql.=" order by i.inc_year desc,i.inc_month desc,e.Name";
//  echo $sql;exit;
...
$rsltBank = db_query($sqlBank);
$arrBank = array();
while ($row_bank = db_fetch_assoc($rsltBank)) {
...
while ($trans_row = db_fetch($trans_result)) {
    if(isset($_GET['loop']) && $_GET['loop']=="success"){
...
//date('M',strtotime($inc_year."-".$inc_month."-01"));
    $pdf->AddPage();
    $pdf->SetLineWidth(.5);
    ...
    $filePath = $path_to_root."/images/company/".$company_id.".jpg";
    if (is_file($filePath)) {
        $filePath = $path_to_root."/images/company/".$company_id.".jpg";
    } else {
        $company_id = '8';
    ...
    $pdf->image($path_to_root."/images/company/".$company_id.".jpg",7,5,null,33);
    ...
    $pdf->Cell(120,7,get_dimension_name($trans_row['division_id'])."-".get_dimension_name($trans_row['sub_division_id']),0,0,'L');
    ...
    $pdf->Cell(30,7,price_format($trans_row['inc_general'] / $rate),0,0,'R');
    ...
    $pdf->Cell(30,7,price_format($eligible_inc_general / $rate),0,0,'R');
    ...
    $pdf->Cell(30,7,price_format($trans_row['inc_special'] / $rate),0,0,'R');
    ...
    $pdf->Cell(30,7,price_format($trans_row['inc_project'] / $rate),0,0,'R');
    ...
    $pdf->Cell(30,7,price_format($eligible_inc_project / $rate),0,0,'R');
    ...
    $pdf->Cell(30,7,price_format($trans_row['job_full'] / $rate),0,0,'R');
    ...
    if ($trans_row['div_deduction'] > 0) {
        ...
        //$pdf->Cell(30,7,price_format(-$trans_row['div_deduction'] / $rate),0,0,'R');
        //$alloted_total = $alloted_total - ($trans_row['div_deduction'] / $rate);
    } else
        $pdf->Cell(30,7,'0.00',0,0,'R');
    ...
    if ($trans_row['div_hold'] > 0) {
         $pdf->Cell(30,7,'0.00',0,0,'R');
        //$pdf->Cell(30,7,price_format(-$trans_row['div_hold'] / $rate),0,0,'R');

        //$alloted_total = $alloted_total - ($trans_row['div_hold'] / $rate);
    } else
        $pdf->Cell(30,7,'0.00',0,0,'R');
    ...
    if ($trans_row['div_release'] > 0) {
      ...
      if ($trans_row['div_release'] > 0) {
        $pdf->Cell(30,7,price_format($trans_row['div_release'] / $rate),0,0,'R');
    ...
    $pdf->Cell(30,7,price_format($trans_row['other_divisions'] / $rate),0,0,'R');
    ...
    $pdf->Cell(30,8,price_format($alloted_total),0,0,'R');
    $pdf->Cell(30,8,price_format($eligible_total),0,0,'R');
    ...
    $pdf->Cell(30,7,price_format($trans_row['royalty'] / $rate),0,0,'R');
    ...
    $pdf->Cell(30,7,price_format($trans_row['petty_cash'] / $rate),0,0,'R');
    ...
    $pdf->Cell(30,7,price_format($trans_row['deduction1'] / $rate),0,0,'R');
    ...
    $pdf->Cell(30,7,price_format($trans_row['deduction2'] / $rate),0,0,'R');
    ...
    $pdf->Cell(30,7,price_format($trans_row['bmi_hold'] / $rate),0,0,'R');
   ...
   $pdf->Line(18,215,193,215);
    ...
    /* $y= $y+7;
      $pdf->SetXY(25, $y);
      $pdf->Cell(60,7,'Ticket Deduction',0,0,'R');
      $pdf->Cell(30,7,price_format($trans_row['bmi_hold']/$rate),0,0,'R');
      $alloted_total  =$alloted_total+($trans_row['bmi_hold']/$rate); */
    ...
    $pdf->Cell(30,10,price_format($eligible_total),0,0,'R');

    $y = $y + 7;
    $pdf->SetLineWidth(.4);
    $pdf->SetFont('Times','B',13);
    //$pdf->Rect(120,100,65,120);
