<?php

require_once "jwt_auth.php";
$auth = requireAuth();
$userId = $auth->uid;

// Take values from POST
$date_report    = trim($_POST['date']     ?? '');
$job_no         = trim($_POST['job_no']   ?? '');
$taskname       = trim($_POST['taskname'] ?? '');
$act_time       = trim($_POST['act_time'] ?? '');
$description    = trim($_POST['description'] ?? '');

// Validate fields
if (empty($date_report) || empty($job_no) || empty($taskname) || empty($act_time)) {
    echo json_encode([
        "status" => false, 
        "message" => "Required fields missing: date, job_no, taskname, act_time"
    ]);
    exit;
}


require_once "connect.inc.php";

$status = "100"; // As per instruction status=100

// Get the next row_id for this user on this date
$rwId = $mysqli->prepare("SELECT row_id FROM `tbl_workreports` WHERE date_report=? AND user_id=? ORDER BY row_id DESC LIMIT 1");
$rwId->bind_param("si", $date_report, $userId);
$rwId->execute();
$rwIdResult = $rwId->get_result();
$row_id = 0;
if ($row = $rwIdResult->fetch_assoc()) {
    $row_id = (int)$row['row_id'];
}
$row_id += 1;

$mainType=1;

// Insert into tbl_workreports
$sql = "INSERT INTO tbl_workreports 
    (user_id, date_report, row_id, taskname, job_no, act_time, description, status, main_type) 
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";

$stmt = $mysqli->prepare($sql);
if (!$stmt) {
    echo json_encode(["status" => false, "message" => "Prepare failed: " . $mysqli->error]);
    exit;
}

$stmt->bind_param(
    "isisssssi",
    $userId, $date_report, $row_id, $taskname, $job_no, $act_time, $description, $status, $mainType
);

if ($stmt->execute()) {
    echo json_encode([
        "status" => true,
        "message" => "Job added successfully."
    ]);
} else {
    echo json_encode([
        "status" => false,
        "message" => "Failed to add job: " . $stmt->error
    ]);
}

$stmt->close();
?>